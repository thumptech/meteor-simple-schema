...
describe('autoValue', function () {
    describe('has correct information in function context', function () {
        it('empty', async function () {
            ...
            const schema = new SimpleSchema({
                ...
                bar: {
                    ...
                    autoValue() {
                        results = this;
                        results.foo = this.field('foo');
                        results.fooSibling = this.siblingField('foo');
                        results.fooParent = this.parentField();
                    },
                },
            });
            await schema.clean({});
            expect(results.key).to.equal('bar');
            expect(results.closestSubschemaFieldName).to.equal(null);
            expect(results.isSet).to.equal(false);
            expect(results.value).to.equal(undefined);
            expect(results.operator).to.equal(null);
            expect(results.foo).to.deep.equal({
                isSet: false,
                operator: null,
                value: undefined,
            });
            expect(results.fooSibling).to.deep.equal({
                isSet: false,
                operator: null,
                value: undefined,
            });
            expect(results.fooParent).to.deep.equal({
                isSet: false,
                operator: null,
                value: undefined,
            });
        ...
        it('async normal other key', async function () {
            let results = {};
            const schema = new SimpleSchema({
                foo: {
                    type: String,
        ...
        it('normal self and other key', async function () {
            ...
            expect(results.isSet).to.equal(true);
            expect(results.value).to.equal(true);
            expect(results.operator).to.equal(null);
            expect(results.foo).to.deep.equal({
                isSet: true,
                operator: null,
        ...
        it('closestSubschemaFieldName set', async function () {
            ...
            expect(result.key).to.equal('dig.dug');
            expect(result.closestSubschemaFieldName).to.equal('dig');
        });
        ...
        it('normal self and no other key with unset', async function () {
            ...
            const schema = new SimpleSchema({
                ...
                bar: {
                    ...
                    autoValue() {
                        result = this;
                        result.foo = this.field('foo');
                        result.fooSibling = this.siblingField('foo');
                        result.fooParent = this.parentField();
                        this.unset();
                    },
                },
            });
            ...
            expect(result.isSet).to.equal(true);
            expect(result.value).to.equal(false);
            expect(result.operator).to.equal(null);
            expect(result.foo).to.deep.equal({
                isSet: false,
                operator: null,
                value: undefined,
            });
            expect(result.fooSibling).to.deep.equal({
                isSet: false,
                operator: null,
                value: undefined,
            });
            expect(result.fooParent).to.deep.equal({
                isSet: false,
                operator: null,
                value: undefined,
            });
        ...
        it('$set self and no other key', async function () {
            ...
            expect(result.isSet).to.equal(true);
            expect(result.value).to.equal(false);
            expect(result.operator).to.equal('$set');
            expect(result.foo).to.deep.equal({
                isSet: false,
                operator: null,
                value: undefined,
        ...
        it('adds $set when missing', async function () {
            ...
            expect(result.isSet).to.equal(false);
    ...
    it('async content AutoValues', async function () {
        function myPromise() {
            return new Promise((resolve, reject) => {
                // Simulate an asynchronous operation (e.g., an API call)
                setTimeout(() => {
                    resolve(new Date('2017-01-01T00:00:00.000Z'));
                }, 1);
            });
        };
        const schema = new SimpleSchema({
    ...
    it('async array of objects autoValues', async function () {
        function myPromise() {
            return new Promise((resolve, reject) => {
                // Simulate an asynchronous operation (e.g., an API call)
                setTimeout(() => {
    ...
    it('async $each in autoValue pseudo modifier', async function () {
            ...
            psuedoEach: {
                ...
                async autoValue() {
                    if (this.isSet && this.operator === '$set') {
                        return {
                            $push: {
                                $each: await myPromise(this.value),
                            },
                        };
                    }
                },
    ...
    it('async simple autoValues', async function () {
        function myPromise(value) {
            return new Promise((resolve, reject) => {
                // Simulate an asynchronous operation (e.g., an API call)
                setTimeout(() => {
        ...
        const schema = new SimpleSchema({
            ...
            someDefault: {
                type: SimpleSchema.Integer,
                async autoValue() {
                    if (!this.isSet) return await myPromise(5);
                },
            },
            updateCount: {
                type: SimpleSchema.Integer,
                async autoValue() {
                    if (!this.operator) return await myPromise(0);
                    return  await myPromise({$inc: 1});
                },
            },
            firstWord: {
                type: String,
                ...
                async autoValue() {
                    const content = this.field('content');
                    if (content.isSet) return await myPromise(content.value.split(' ')[0]);
                    this.unset();
                },
            },
        });
